<!--
 ...
 @Desc:量算工具面板
-->
span { margin: 0 5px; border-radius: 4px; color: #2988CC; }
	...
	<div id="measurePanel">
		<el-container>
			<el-header id="measureHead">
				...
				<span style="margin: 0 10px" @click="$emit('closeEvent')"></span>
			...
			<el-main class="graphic-draw-main">
				<ul>
					...
					<li>
						<i class="iconfont icon-delete" title="清除全部" @click="removeAll"></i>
		...
		<div class="edit-class" v-show="mode === 'distance'">
			<el-select size="mini" v-model="distaceMode" allow-create filterable title="字号" default-first-option placeholder="请选择">
...
export default {
	...
	methods: {
		...
		addEventListener() {
			...
			_this.handler.setInputAction(onMousemove, Cesium.ScreenSpaceEventType.MOUSE_MOVE);
		...
		toggleClick(mode) {
			...
			switch (mode) {
				case 'distance':
					...
					break;
				case 'area':
					window.graphicManager = new PolygonGraphic(window.viewer, {
					...
					break;
				case 'height':
					window.graphicManager = new HeightGraphic(window.viewer, {
						positions: this.positions,
						material: color,
						width: 3
					});
