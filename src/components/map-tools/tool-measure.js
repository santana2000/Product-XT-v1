        ...
        if (this.pointArr.length === 1) {
            ...
            if (x === 'rightClick') {
                ...
                const entityPolygon = this.viewer.entities.add({
                    polygon : {
                        hierarchy : this.pointArr,
                        material : Cesium.Color.RED,
                    }
                });

                // draw polygon...
                // get sum area by turf
            } else if (x === 'leftClick') {
                //draw polyline...
                const entityLine = this.viewer.entities.add({
                    polyline: this.options.polyline,
    ...
    addLine(event) {
        ...
        if (tempPoint) {
            ...
            if (this.pointArr.length == 1) {
                ...
                //画两条
                this.tempLineArr[0] = this.pointArr[0];
                this.tempLineArr[1] = tempPoint;
                this.tempLineArr[2] = this.pointArr[this.pointArr.length - 1];
            } else {
                return;
            }
            let updatePoi = () => {  //如何抽取为公共函数？
