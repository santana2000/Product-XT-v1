...
const PolylineGraphic = (function() {
	function _(viewer, options = {}, mode = 'space') {
		this.viewer = viewer;
		this.mode = mode;
		viewer.scene.postProcessStages.fxaa.enabled = false;
		options.clampToGround = mode === 'surface';
		this.options = {
			polyline: options
		};
	...
	_.prototype.setMode = function(mode) {
		this.mode = mode;
		this.entity.polyline.clampToGround = mode === 'surface';
	};
	...
	_.prototype.createLabel = function() {
		...
		if (this.mode === 'surface') {
			length =
				this.getDistance() > 10 * 1000
					? '总长度' + (this.getDistance() / 1000).toFixed(2) + 'km'
					: '总长度' + this.getDistance().toFixed(2) + 'm';
		} else if (this.mode === 'space') {
			length =
				this.getSpaceDistance() > 10 * 1000
					? '总长度' + (this.getSpaceDistance() / 1000).toFixed(2) + 'km'
					: '总长度' + this.getSpaceDistance().toFixed(2) + 'm';
		}
	...
	_.prototype.getSpaceDistance = function() {
		let dis = 0;
		for (let i = 1; i < this.positions.length; i++) {
			const s = this.positions[i - 1];
			const e = this.positions[i];
...
const HeightGraphic = (function() {
	...
	_.prototype.createAssitGraphic = function() {
		...
		const tmp = Cesium.Cartesian3.fromRadians(endC.longitude, endC.latitude, startC.height);
...
const PolygonGraphic = (function() {
	...
	_.prototype.createLabel = function() {
		const area = this.getArea() < 0.1 ? (this.getArea() * 1000 * 1000).toFixed(2) + 'm²' : this.getArea().toFixed(2) + 'km²';
		const label = labelStyle;
		label.text = '面积:' + area;
		return label;
	};
